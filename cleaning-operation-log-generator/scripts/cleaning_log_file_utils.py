"""Shared file helpers for cleaning operation log generation."""
...
def load_csv(file_path: str | Path) -> pd.DataFrame:
    """Load a CSV file, returning an empty DataFrame for an empty file."""
...
def load_json_any(file_path: str | Path) -> Any:
    """Load JSON content from a UTF-8 file."""
    path = _require_existing_file(file_path)
    try:

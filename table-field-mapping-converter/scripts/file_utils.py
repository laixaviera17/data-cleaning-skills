"""Shared file helpers for table field mapping conversion."""
...
def load_mapping_csv(file_path: str | Path) -> pd.DataFrame:
    """Load field mapping rules from a CSV file."""
    path = _require_existing_file(file_path)
    try:
        return pd.read_csv(path, encoding=DEFAULT_ENCODING, keep_default_na=False)
    except pd.errors.EmptyDataError as exc:
        raise RuntimeError(f"字段映射文件为空: {path}") from exc
    except UnicodeDecodeError as exc:
        raise RuntimeError(f"字段映射文件编码错误，请确认是否为 UTF-8: {path}") from exc
    except Exception as exc:
        raise RuntimeError(f"字段映射文件读取失败: {path}") from exc
